...
async function moveTags() {
    ...
    try {
        ...
        const { rows: tags } = await client.query(`
            ...
            WHERE name ILIKE 'en contacto' 
               OR name ILIKE 'REMARKETING 05%'
        `);
        
        const sourceTags = tags.filter(t => t.name.toLowerCase() === 'en contacto');
        const targetTag = tags.find(t => t.name.toUpperCase() === 'REMARKETING 05');

        if (sourceTags.length === 0) {
            console.error('❌ No se encontró la etiqueta "en contacto". Etiquetas devueltas:', tags);
            return;
        }
        if (!targetTag) {
