...
async function moveTags() {
    ...
    try {
        ...
        // 1. Get exact tag names (handling potential typos)
        const { rows: tags } = await client.query(`
            SELECT id, name FROM tags 
            WHERE name ILIKE '%escribir%'
               OR name ILIKE '%ecribir%'
               OR name ILIKE 'REMARKETING 00%'
        `);
        
        const sourceTags = tags.filter(t => t.name.toLowerCase().includes('escribir') || t.name.toLowerCase().includes('ecribir'));
        const targetTag = tags.find(t => t.name.toUpperCase() === 'REMARKETING 00');

        if (sourceTags.length === 0) {
            console.error('❌ No se encontró ninguna etiqueta parecida a "no escribir mas". Etiquetas disponibles en db:', tags);
            return;
        }
        if (!targetTag) {
