...
async function moveTags() {
    ...
    try {
        ...
        const { rows: tags } = await client.query(`
            ...
            WHERE name ILIKE '%nuevo abril%' 
               OR name ILIKE 'REMARKETING 02%'
        `);
        
        const sourceTags = tags.filter(t => t.name.toLowerCase().includes('nuevo abril'));
        const targetTag = tags.find(t => t.name.toUpperCase() === 'REMARKETING 02');

        if (sourceTags.length === 0) {
            console.error('❌ No se encontró la etiqueta "nuevo abril". Etiquetas devueltas:', tags);
            return;
        }
        if (!targetTag) {
