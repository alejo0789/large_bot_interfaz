...
async function moveTags() {
    ...
    try {
        ...
        const { rows: tags } = await client.query(`
            ...
            WHERE name ILIKE '%nuevo junio%' 
               OR name ILIKE '%nuevo julio%' 
               OR name ILIKE 'REMARKETING 02%'
        `);
        
        const sourceTags = tags.filter(t => t.name.toLowerCase().includes('nuevo junio') || t.name.toLowerCase().includes('nuevo julio'));
        const targetTag = tags.find(t => t.name.toUpperCase() === 'REMARKETING 02');

        if (sourceTags.length === 0) {
            console.error('❌ No se encontraron las etiquetas "nuevo junio" ni "nuevo julio". Etiquetas devueltas:', tags);
            return;
        }
        if (!targetTag) {
            console.error('❌ No se encontró la etiqueta "REMARKETING 02". Etiquetas devueltas:', tags);
            return;
        }
        ...
        // Select distinct phones from the source tags
        ...
        console.log(`✅ Se asignó la etiqueta "REMARKETING 02" a ${insertRes.rowCount} contactos que no la tenían.`);
