...
async function moveTags() {
    ...
    try {
        ...
        const { rows: tags } = await client.query(`
            ...
            WHERE name ILIKE '%remarketing 40%' 
               OR name ILIKE 'REMARKETING 00%'
        `);
        
        const sourceTags = tags.filter(t => t.name.toLowerCase().includes('remarketing 40'));
        const targetTag = tags.find(t => t.name.toUpperCase() === 'REMARKETING 00');

        if (sourceTags.length === 0) {
            console.error('❌ No se encontró la etiqueta "remarketing 40". Etiquetas devueltas:', tags);
            return;
        }
        if (!targetTag) {
            console.error('❌ No se encontró la etiqueta "REMARKETING 00". Etiquetas devueltas:', tags);
            return;
        }
