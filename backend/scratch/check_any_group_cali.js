...
async function checkAnyGroupMessages() {
    const pool = new Pool({
    ...
    try {
        ...
        const { rows } = await caliPool.query("SELECT conversation_phone, text_content, timestamp FROM messages WHERE conversation_phone LIKE '%@g.us' ORDER BY timestamp DESC LIMIT 5");
        console.log('Last 5 group messages in Cali DB (ANY group):', rows);
