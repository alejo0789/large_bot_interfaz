...
const caliUrl = 'postgresql://neondb_owner:npg_LYBzGw64JDWh@ep-withered-term-a4nrhhk1-pooler.us-east-1.aws.neon.tech/chatbot_db?sslmode=require&channel_binding=require';

const caliPool = new Pool({ connectionString: caliUrl });
...
async function check() {
    try {
        const query = `
            ...
            combined AS (
                ...
                SELECT 
                    ...
                    true as is_human
                FROM agents a
                LEFT JOIN message_stats ms ON a.id::text = ms.activity_id OR a.name = ms.activity_id
                LEFT JOIN agenda_stats ags ON a.id::text = ags.agent_id OR a.name = ags.agent_id
        ...
        `;
        const res = await caliPool.query(query);
